 ...
 */
@Component
public class ResetAppConfigRabbitMQReceiver2 implements IMQMsgReceiver {
     ...
     *   如果queue包含多个消费者，
     *   【例如，manager和payment的监听器是名称相同的queue下的消费者（Consumers） 】， 两个消费者是工作模式且存在竞争关系， 导致只能一个来消费。
     * 解决：
    ...
    @RabbitListener(
            bindings = {@QueueBinding(
                    ...
                    exchange = @Exchange(name = RabbitMQConfig.FANOUT_EXCHANGE_NAME_PREFIX + ResetAppConfigMQ.MQ_NAME,type = ExchangeTypes.FANOUT ))} )
    public void receiveMsg(String msg,Message message, Channel channel){
        System.out.println("我是消费者2号");
        mqReceiver.receive(ResetAppConfigMQ.parse(msg), message, channel);
    }
